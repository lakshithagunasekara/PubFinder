...
from utils.extract_results import extract_search_results_for_source, load_search_results
...
from utils.filtering import filter_duplicates, filter_by_cites, \
    filter_keywords_with_gpt, filter_by_optional_keywords, filter_abstract_by_mandatory_keywords, \
    filter_titles_by_mandatory_keywords
import config
...
output_directory, sources_directory, archive_directory = prepare_directories(clear_directories=not config.reuse["sources"])
...
if config.allow_filter_by_optional_keywords:
    ...
    journal_data_df = filter_keywords_with_gpt(journal_data_df, config.mandatory_filters, config.optional_filter,
                                               output_directory, batch_size=10)
